...
public class Room {
    ...
    public List<UUID> getOtherPlayers(UUID playerId, SocketIOServer server) {
        System.out.println("Getting winners ");
        List<UUID> collect = idToPlayerName.keySet().stream().filter(key -> !key.equals(playerId)).collect(Collectors.toList());
